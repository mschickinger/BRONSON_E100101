...
vwcm_job = batch(mycluster, @par_vwcm_v1, 1, { ch1, ch2, pos_in_frame, path_out} ...
    ,'CaptureDiary',true, 'CurrentDirectory', '.', 'Pool', 31 ...
...
vwcm_output = vwcm_job.fetchOutputs;
...
%% Mapping - map fitted coordinates on other channel
if mapping
    for m = 1:N_movie
    for i = 1:size(vwcm_result{m},1)
...
%% save data
save([path_out filesep 'data_all.mat']);
