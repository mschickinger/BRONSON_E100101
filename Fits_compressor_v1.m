...
% Set movie index (to be removed later)
m = 1;
...
%for m = 1:N_movie
    for ch = 1:2
        for i = 1:floor(length(movies{m,ch}.frames)/movies{m,ch}.N_frame_per_fits)
            ...
            fitswrite(mov_out, [pname filesep movies{m,ch}.fname{1}(1:end-5) '_compressed' sprintf('%02d',i) '.fits'])
        end
        ...
        display(['Writing compressed .fits file #' num2str(i+1) ' of ' ...
            num2str(ceil(length(movies{m,ch}.frames)/movies{m,ch}.N_frame_per_fits)) ...
            ' in movie #' num2str(m) ', channel ' num2str(ch)])
        fitswrite(mov_out, [pname filesep movies{m,ch}.fname{1}(1:end-5) '_compressed' sprintf('%02d',i+1) '.fits'])
    end
%end
